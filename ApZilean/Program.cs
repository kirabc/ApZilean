...
namespace ApZilean
{
    class Program
    {
        ...
        private static void ComboVombo()
        {
            ...
            if (ise && E.IsReady() && E.IsInRange(enemy) && !Q.IsReady()) E.Cast(enemy);

            if (isq && Q.IsReady() && W.IsReady() && Q.IsInRange(enemy)) Q.Cast(Q.GetPrediction(enemy).CastPosition);

            if (isw && !Q.IsReady() && W.IsReady()) W.Cast();
